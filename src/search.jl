...
function custo(x::Vector{Float64})
    ...
    erro_init = sum(abs.(x[1][1:floor(Integer,size/3)]))/(maximum(abs.(x[1]))) +
    sum(abs.(x[2][1:floor(Integer,size/3)]))/(maximum(abs.(x[2])))
    #erro_init = sum(abs.(erro1[1:floor(Integer,size/3)] - r1))/(abs(maximum(abs.(erro1- r1)))) + sum(abs.(erro2[1:floor(Integer,size/3)] - r2))/(abs(maximum(abs.(erro2- r2))))
    erro_end = sum(abs.(erro1[2*floor(Integer,size/3):end]))/maximum(abs.(erro1)) + sum(abs.(erro2[2*floor(Integer,size/3):end]))/maximum(abs.(erro2))
    jerk = sum(abs.(j[1]))/maximum(abs.(j[1])) + sum(abs.(j[2]))/maximum(abs.(j[2]))
...
N = 4
println("iniciando")
result, fitness, cnt = ga(custo, N; initPopulation = (n -> rand(n).*[1000., 100., 100., 10.]), populationSize = 50, ɛ = 0.1, selection = sus, crossover = intermediate(0.25), mutation = domainrange(fill(0.5,N)), iterations = 10)
